...
class TrainModule:
    ...
    def __init__(self, args, logger):
        ...
        self.metrics = {
            ...
            'share_lss': tf_metrics.Mean(name='share_lss'), # share修改
            'share_acc': tf_metrics.CategoricalAccuracy(name='share_acc')
        }
        self.share_lss = []
        self.share_acc = [] # share 修改
        self.k = 0 # 轮数
        self.local_lss = []
    ...
    def evaluate_loacl(self):
        tf.keras.backend.set_learning_phase(0)
        for i in range(0, len(self.task['shareX']), self.args.batch_size_test):
            x_batch = self.task['shareX'][i:i+self.args.batch_size_test]
            y_batch = self.task['shareY'][i:i+self.args.batch_size_test]
        ...
        sharelss, shareacc = self.measure_performance('share_lss', 'share_acc')

        return sharelss, shareacc
    ...
    def fedshare(self, updates, lss, acc):
        ...
        for i in acc:
            sumAcc += i
        for i in range(joinTrain):
            w.append(round(float((sumLss - copy.deepcopy(ori[i])) / ((len(updates) - 1) * sumLss)), joinTrain))
